...
* **reset_pin** (*任意*, GPIO出力ピン): BP35C1-J11-T01 の RESETN 端子に接続した GPIO ピンを指定する。起動時・再接続時にハードウェアリセット(Lowパルス)を行い、モジュールを未起動状態に戻す。ハードウェア仕様書で RESETN のホスト制御が前提とされているため設定を推奨。未指定時はソフトウェアリセット(0x00D9)のみで動作する
